...
// Hàm helper để hiển thị thông báo lỗi và đổi màu khung ô nhập liệu sang đỏ
function showInputError(inputEl, message) {
    inputEl.style.borderColor = 'var(--primary-red)';
    inputEl.style.boxShadow = '0 0 8px rgba(225, 29, 44, 0.3)';
    
    // Kiểm tra xem đã có thẻ báo lỗi phía sau chưa, nếu chưa có thì tạo mới
    let errorEl = inputEl.parentNode.querySelector('.error-message-text');
    if (!errorEl) {
...
// Hàm helper để xóa bỏ trạng thái lỗi khi người dùng nhập đúng
...
// Thiết lập các bộ lắng nghe sự kiện Validate khi Blur và Input cho từng ô
function setupFormValidation() {
    // 1. Ô Đăng nhập: Tên tài khoản
    const loginUser = document.getElementById('loginUser');
    if (loginUser) {
        loginUser.addEventListener('blur', () => {
            if (!loginUser.value.trim()) showInputError(loginUser, 'Bạn chưa nhập tên đăng nhập!');
        });
        loginUser.addEventListener('input', () => {
            if (loginUser.value.trim()) clearInputError(loginUser);
        });
    }

    // 2. Ô Đăng nhập: Mật khẩu
    const loginPass = document.getElementById('loginPass');
    if (loginPass) {
        loginPass.addEventListener('blur', () => {
            if (!loginPass.value) showInputError(loginPass, 'Bạn chưa nhập mật khẩu!');
        });
        loginPass.addEventListener('input', () => {
            if (loginPass.value) clearInputError(loginPass);
        });
    }

    // 3. Ô Đăng ký: Tên tài khoản mới
    const regUser = document.getElementById('regUser');
    if (regUser) {
        regUser.addEventListener('blur', () => {
            if (!regUser.value.trim()) showInputError(regUser, 'Bạn chưa nhập tên tài khoản mới!');
        });
        regUser.addEventListener('input', () => {
            if (regUser.value.trim()) clearInputError(regUser);
        });
    }

    // 4. Ô Đăng ký: Email liên hệ
    const regEmail = document.getElementById('regEmail');
    if (regEmail) {
        regEmail.addEventListener('blur', () => {
            const val = regEmail.value.trim();
            if (!val) {
                showInputError(regEmail, 'Bạn chưa nhập email!');
            } else if (!isValidEmailFormat(val)) {
                showInputError(regEmail, 'Định dạng email không hợp lệ (Ví dụ phải có @ và tên miền)!');
            }
        });
        regEmail.addEventListener('input', () => {
            if (regEmail.value.trim() && isValidEmailFormat(regEmail.value.trim())) {
                clearInputError(regEmail);
            }
        });
    }

    // 5. Ô Đăng ký: Mật khẩu khởi tạo
    const regPass = document.getElementById('regPass');
    if (regPass) {
        regPass.addEventListener('blur', () => {
            const val = regPass.value;
            if (!val) {
                showInputError(regPass, 'Bạn chưa nhập mật khẩu khởi tạo!');
            } else if (val.length < 6) {
                showInputError(regPass, 'Mật khẩu phải tối thiểu có từ 6 ký tự trở lên!');
            }
        });
        regPass.addEventListener('input', () => {
            if (regPass.value && regPass.value.length >= 6) {
                clearInputError(regPass);
            }
        });
    }
}
// Khởi chạy kích hoạt tính năng kiểm tra lỗi form
setupFormValidation();
...
function seedData(){
    ...
    if(get(STORE.TRAINERS).length===0){
        set(STORE.TRAINERS, [
            { id: 'T-801', name: 'Nguyễn Trần Tiến', special: 'Cơ bắp & Siết mỡ', phone: '0912345678', status: 'active' },
            { id: 'T-802', name: 'Lê Hoàng Trung', special: 'Kickboxing & Cardio', phone: '0988887777', status: 'busy' }
        ]);
    }
...
function switchView(viewName) {
    ...
    // --- VIEW: TRAINERS ---
        ...
        contentEl.innerHTML = `<div class="trainers-grid" id="trainersGridBody"></div>`;
...
function renderMemberTable(dataArray) {
    ...
    tbody.innerHTML = dataArray.map(m => {
        ...
        if(isPowerAdmin) {
            ...
            // Thiết kế giao diện chuyên nghiệp cho User (Nhân viên lễ tân): Không hiển thị nút xóa, thay bằng nút gửi yêu cầu hỗ trợ nhanh
...
function renderTrainersGrid(trainersArray) {
    ...
    target.innerHTML = trainersArray.map(t => `
        <div class="trainer-card" style="background: #0f0f0f; border: 1px solid #222; border-radius: 12px; padding: 20px;">
            <h4 style="color:#fff; font-size:1.1rem; margin-bottom:5px;">PT: ${t.name}</h4>
            <div style="color:var(--primary-red); font-size:0.85rem; font-weight:600; margin-bottom:12px;">Chuyên môn: ${t.special}</div>
            <div style="font-size:0.85rem; color:#888;"><i class="fas fa-phone-alt"></i> Liên hệ: ${t.phone}</div>
        </div>
    `).join('');
}
...
window.deletePackage = function(planId) {
    if(get(STORE.CURRENT_USER).role === 'user') return alert('Bạn không có quyền thực hiện chức năng xóa!');
...
window.deleteMember = function(memberId) {
    if(get(STORE.CURRENT_USER).role === 'user') return alert('Bạn không có quyền thực hiện chức năng xóa hội viên!');
